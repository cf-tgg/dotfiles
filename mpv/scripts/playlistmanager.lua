-- ~& [ playlistmanager.lua ] [ Last Update: 2025-01-01 20:31 ]
...
local settings = {
	...
	show_playlist_on_file_load = true,
	...
	peek_respect_display_timeout = true,
...
function save_playlist(filename)
	...
	if name == nil then
		if settings.playlist_save_filename == nil or settings.playlist_save_filename == "" then
			...
			name = datestring .. "_playlist-size_" .. length .. ".m3u8"
		else
			local prefix = settings.playlist_save_filename
			-- local date = os.date("*t")
			-- local tdatestring = ("%02d%02d%02d%02d"):format(date.year, date.month, date.day, date.hour)
			name = prefix .. ".m3u8"
			-- name = prefix .. "." .. tdatestring .. ".m3u8"
		end
	end
