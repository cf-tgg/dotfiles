...
mp.add_forced_key_binding(nil, "select-playlist", function()
  ...
  local trailing_slash_pattern = mp.get_property("platform") == "windows" and "[/\\]+$" or "/+$"
  ...
  for i, entry in ipairs(mp.get_property_native("playlist")) do
    ...
    if not playlist[i] or show ~= "title" then
      ...
      if not playlist[i]:find("://") then
        playlist[i] = select(2, utils.split_path(playlist[i]:gsub(trailing_slash_pattern, "")))
      end
    end
...
mp.add_forced_key_binding(nil, "select-track", function()
  ...
  for i, track in ipairs(mp.get_property_native("track-list")) do
    tracks[i] = (track.image and "Image" or track.type:sub(1, 1):upper() .. track.type:sub(2))
        .. ": "
        .. format_track(track)
  end
...
mp.add_forced_key_binding(nil, "select-secondary-sid", function()
  select_track("secondary-sid", "sub", "Select a secondary subtitle:", "No available subtitles.")
end)

mp.add_forced_key_binding(nil, "select-aid", function()
  select_track("aid", "audio", "Select an audio track:", "No available audio tracks.")
end)

mp.add_forced_key_binding(nil, "select-vid", function()
  select_track("vid", "video", "Select a video track:", "No available video tracks.")
end)
...
mp.add_forced_key_binding(nil, "select-binding", function()
  ...
  for _, binding in pairs(mp.get_property_native("input-bindings")) do
    if
        binding.priority >= 0
        and (
          bindings[binding.key] == nil
          or (bindings[binding.key].is_weak and not binding.is_weak)
          or (
            binding.is_weak == bindings[binding.key].is_weak
            and binding.priority > bindings[binding.key].priority
          )
        )
    then
...
mp.add_forced_key_binding(nil, "show-properties", function()
  ...
  mp.set_property("msg-level", msg_level_backup == "" and "cplayer=no" or msg_level_backup .. ",cplayer=no")
    ...
    submit = function(i)
      if mp.get_property_native("vo-configured") then
        mp.commandv(
          "expand-properties",
          "show-text",
          (#properties[i] > 100 and "${osd-ass-cc/0}{\\fs9}${osd-ass-cc/1}" or "") .. "$>" .. properties[i],
          20000
        )
      else
