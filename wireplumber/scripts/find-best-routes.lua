	...
	execute = function(event)
		...
		for _, device_id in ipairs(active_ids) do
			...
			for _, ri in pairs(dev_info.route_infos) do
				if
					cutils.arrayContains(ri.devices, device_id)
					and (ri.profiles == nil or cutils.arrayContains(ri.profiles, dev_info.active_profile))
				then
			...
			if route then
				selected_routes[tostring(device_id)] = Json.Object({ index = route.index }):to_string()
			end
